...
/**
 * Boolean flag that indicates that this process is about to exit or has already exited.
 *
 * @type {boolean}
 * @private
 */
...
Phantom.prototype.run = function (args, fn) {
    if (this._isDisposed) {
        throw new Error("Cannot run function: phantom instance is already disposed");
    }
 ...
 *
 * Please note: It is possible that this function is called multiple times in error conditions. Thus it is important
 * to guard all calls so that they can be executed multiple times.
 *
 * @private
 */
...
Phantom.prototype._afterExit = function () {
    ...
    // Send kill signal just in case the process was not exited cleanly
