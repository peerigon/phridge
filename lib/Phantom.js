...
Phantom.prototype.openPage = function (url) {
    ...
    return page.run(function (params, resolve, reject) {
        this.open(params.url, function onPageLoaded(status) {
            if (status !== "success") {
                return reject(new Error("Cannot load " + params.url + ": Phantomjs returned status " + status));
            }
            resolve();
        });
