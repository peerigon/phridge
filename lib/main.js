...
function create(config) {
    return new Promise(function create(resolve, reject) {
        ...
        function startPhantom() {
            ...
            //console.log("startPhantom", !port || (config && !configPath));
            ...
            //console.log(phantomjs.path, args);
            child = childProcess.execFile(phantomjs.path, [startScript, port]);

            child.stdout.once("data", function (data) {
                data = data.slice(0, 4);    // remove line-breaks from console.log()
                ...
                if (data === "true") {
                    ...
                    reject(new Error("Cannot start phantomjs server: Sub-process didn't respond with an ok"));
                }
            });
        }
    });
